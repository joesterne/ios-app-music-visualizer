...
(async()=>{
 const h=harness();assert.equal(h.cards.length,13);for(const card of h.cards){card.click();assert.equal(h.elements.get('visual-name').textContent,card.name==='Tron'?'Light cycles':card.name)}
 ...
 for(const t of [0,280/38,500/38,640/38,1000000]){
  ...
  assert(h.run('marioRects.length>100&&marioRects.length<2000'));
  assert(h.run('marioRects.every(r=>r.every(Number.isFinite))'));
 }
 for(const t of [0,20,1000000])h.run(`render($('visual'),'spaceFlight',${t},bands,wave)`);
 ...
 console.log('PASS: 13 galleries, 5 palettes, settings/immersive close, microphone denial/retry/disconnect, stale permission success/failure, muted analysis graph, local playback/pause/seek/restart/volume.');
